...
const MyList: NextPage = () => {
  ...
  const [mov, setMov] = useState<StateProp>({
    url: "",
    titleText: "",
    desc: "",
    year: "",
  ...
  return (
    <Layout activePage="My List">
      <ListConatiner>
        ...
        {list[0] ? (
          <ListGrid>
            {list.map((item: Omit<PopUpProps, "set">) => (
              <MovieBoxStyle
                key={item.id}
                onClick={() => {
                  setMov({
          ...
          <p className="empty">You have no movies / shows saved.</p>
        )}

        {showInfoModal && (
          <InfoModals
            set={setShowInfoModal}
            url={mov.url}
            titleText={mov.titleText}
            year={mov.year}
            score={mov.score}
            id={mov.id}
            desc={mov.desc}
